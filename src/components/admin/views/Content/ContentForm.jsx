...
export default function ContentForm({
  ...
  /* =========================
     파일 State (🔥 수정 핵심)
  ========================= */
  const [originFiles, setOriginFiles] = useState([]); // 기존 서버 파일들
  ...
  const handleAttachFiles = async (e) => {
    ...
    const urls = await uploadFilesWithPresign(imageFiles);
  ...
  const handleRemoveFile = (item, index) => {
    ...
    if (item.isServerFile) {
      ...
      setOriginFiles((prev) => prev.filter((f) => f.fileId !== item.fileId));
    } else {
      // 삭제하려는 신규 파일의 순서를 계산해 addFiles에서 제거
      const newIndex = filePreviews
        .slice(0, index)
        .filter((p) => !p.isServerFile).length;
      setAddFiles((prev) => prev.filter((_, i) => i !== newIndex));
    }
  };
  ...
  return (
    <>
      <div className="animate-in fade-in mx-auto w-full max-w-[900px] space-y-4 lg:w-1/2">
        <button
          onClick={onBack}
        ...
        <div className="space-y-6 rounded-lg border bg-white p-6">
              ...
              <div className="space-y-4">
                ...
                {bannerPreview && (
                  <img
                    src={bannerPreview}
                    alt="배너 미리보기"
                    className="h-auto max-h-64 w-full cursor-pointer rounded bg-gray-50 object-contain"
                    onClick={() => openLightbox([bannerPreview])}
                  />
                )}
              </div>
            )}
          ...
          <div>
            ...
            {filePreviews.length > 0 && (
              <div className="mt-3 grid w-full grid-cols-1 justify-items-center gap-3 sm:grid-cols-2 md:grid-cols-3">
                {filePreviews.map((item, idx) => (
                  <div
                    key={idx}
                    className="relative flex h-64 w-full items-center justify-center overflow-hidden rounded border bg-gray-50"
                  >
                    <img
                      src={item.previewUrl}
                      alt="미리보기"
                      ...
                      onClick={() =>
                        openLightbox(
                          filePreviews.map((p) => p.previewUrl),
                          idx
                        )
                      }
                    />
